...
double HikeList::getPrice(const string &hikeName) const
{
    ...
    auto iter = find_if(hikeList.begin(), endIter, [hikeName](const pair<Hike, double> current)
...
void HikeList::printByLocation(const string &location) const
{
    ...
    while (currentIter != endIter)
    {
        currentIter = find_if(currentIter, endIter, [location](const pair<Hike, double> &pair)
